...
export function ImageSlider({ images, autoPlay = true, interval = 5000, visibleCount = 4 }: ImageSliderProps) {
  ...
  return (
    <section 
      ref={ref}
      className={`relative w-full pt-4 pb-12 bg-white overflow-hidden transition-all duration-1000 ${
      ...
      <div className="w-full overflow-x-hidden px-4">
        ...
        <div 
          className={`grid gap-4 ${
            responsiveVisibleCount === 1 ? 'grid-cols-1' :
            responsiveVisibleCount === 3 ? 'grid-cols-3' :
            'grid-cols-5'
          }`}
        >
          {displayImages.map((image, index) => (
            <div
              ...
              className="relative aspect-[3/4] min-h-[400px] sm:min-h-[500px] md:min-h-[600px] overflow-hidden"
            >
              <OptimizedImage
                src={image}
                alt={`Slide ${currentIndex + index + 1}`}
                className="w-full h-full object-cover"
                sizes={responsiveVisibleCount === 1 ? "100vw" : responsiveVisibleCount === 3 ? "33vw" : "20vw"}
